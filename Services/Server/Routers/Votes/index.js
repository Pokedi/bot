...
voteRouter.post("/topgg", async (req, res) => {
    ...
    try {
        ...
        streak = (CASE WHEN (extract(epoch from now()) - extract(epoch from user_vote.last_voted)) < (extract(epoch from interval '24 hours')) THEN (user_vote.streak + 1) ELSE 0 END),
