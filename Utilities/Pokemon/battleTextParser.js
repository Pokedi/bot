...
var BattleTextParser = function () {
	function BattleTextParser() {
		var perspective = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'p1'; this.p1 = "Player 1"; this.p2 = "Player 2"; this.p3 = "Player 3"; this.p4 = "Player 4"; this.perspective = void 0; this.gen = 9; this.turn = 0; this.curLineSection = 'break'; this.lowercaseRegExp = undefined;
									...
									effectId = function effectId(effect) {
										...
										effect = function effect(_effect) {
											...
											template = function template(type) {
												for (var _len = arguments.length, namespaces = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) { namespaces[_key - 1] = arguments[_key]; } for (var _i4 = 0; _i4 <
													namespaces.length; _i4++) {
																			...
																			switch (cmd) {
																				...
																				case '-start': {
																					...
																					if (_id5 === 'typechange') {
																						...
																						return _line4 + _template10.replace('[POKEMON]', this.pokemon(_pokemon10)).replace('[TYPE]', _arg).
																					...
																					return _line4 + _template15.replace('[POKEMON]', this.pokemon(_pokemon10)).replace('[EFFECT]', this.effect(_effect3)).
																				...
																				case '-end': {
																					...
																					return _line5 + _template17.replace('[POKEMON]', this.pokemon(_pokemon11)).replace('[EFFECT]', this.effect(_effect4)).
																				...
																				case '-ability': {
																					...
																					if (kwArgs.from) {
																						...
																						return _line6 + _template19.replace('[POKEMON]', this.pokemon(_pokemon12)).replace('[ABILITY]', this.effect(ability)).
																							replace('[SOURCE]', this.pokemon(kwArgs.of));
																					}
																				...
																				case '-item': {
																					...
																					var _target = '';
																					if (['magician', 'pickpocket'].includes(_id8)) {
																						var _ref2 =
																					...
																					if (['thief', 'covet', 'bestow', 'magician', 'pickpocket'].includes(_id8)) {
																						...
																						return _line8 + _template23.replace('[POKEMON]', this.pokemon(_pokemon14)).replace('[ITEM]', this.effect(item)).
																					...
																					if (_id8 === 'frisk') {
																						...
																						return _line8 + _template24.replace('[POKEMON]', this.pokemon(kwArgs.of)).replace('[ITEM]', this.effect(item)).
																				...
																				case '-status': {
																					...
																					if ((_kwArgs$from3 = kwArgs.from) != null && _kwArgs$from3.startsWith('item:')) {
																						var _template33 = this.template('startFromItem', status);
																						return _line10 + _template33.replace('[POKEMON]', this.pokemon(_pokemon16)).replace('[ITEM]', this.effect(kwArgs.from));
																					}
																				...
																				case '-singleturn': case '-singlemove': {
																					...
																					return _line12 + _template41.replace('[POKEMON]', this.pokemon(_pokemon18)).replace('[SOURCE]', this.pokemon(kwArgs.of)).
																				...
																				case '-activate': {
																					...
																					if (!_target2 &&
																						['hyperdrill', 'hyperspacefury', 'hyperspacehole', 'phantomforce', 'shadowforce', 'feint'].includes(_id11)) {
																						var _ref3 =
																					...
																					if (_id11 === 'commander') {
																						...
																						var _template50 = this.template('activate', _id11);
																					...
																					if (kwArgs.move || kwArgs.number || kwArgs.item || kwArgs.name) {
																						_template51 = _template51.replace('[MOVE]', kwArgs.move).replace('[NUMBER]', kwArgs.number).
																							replace('[ITEM]', kwArgs.item).replace('[NAME]', kwArgs.name);
																					}
																					return _line15 + _template51.replace('[POKEMON]', this.pokemon(_pokemon19)).replace('[TARGET]', this.pokemon(_target2)).
																						replace('[SOURCE]', this.pokemon(kwArgs.of));
																				}
																				...
																				case '-damage': {
																					...
																					if (kwArgs.from.startsWith('item:')) {
																						...
																						return _line16 + _template53.replace('[POKEMON]', this.pokemon(_pokemon21)).replace('[ITEM]', this.effect(kwArgs.from)).
																							replace('[SOURCE]', this.pokemon(kwArgs.of));
																					}
																				...
																				case '-heal': {
																					...
																					if (_template54) {
																						return _line17 + _template54.replace('[POKEMON]', this.pokemon(_pokemon22)).replace('[SOURCE]', this.pokemon(kwArgs.of)).
																				...
																				case '-boost': case '-unboost': {
																					...
																					return _line18 + _template56.replace(/\[POKEMON\]/g, this.pokemon(_pokemon23)).replace('[STAT]', BattleTextParser.stat(_stat2));
																				}
																				...
																				case '-block': {
																					...
																					var _id14 = BattleTextParser.effectId(_effect14);
																					var _templateId9 = 'block';
																					if (_id14 === 'mist' && this.gen <= 2) _templateId9 = "blockGen" + this.gen;
																					var _template63 = this.template(_templateId9, _effect14);
																					return _line24 + _template63.replace('[POKEMON]', this.pokemon(_pokemon30)).
																						replace('[SOURCE]', this.pokemon(attacker || kwArgs.of)).replace('[MOVE]', _move2);
																				}

																				case '-fail': {
																					var _pokemon31 = args[1], _effect15 = args[2], _stat3 = args[3];
																					var _id15 = BattleTextParser.effectId(_effect15);
																					...
																					if (['desolateland', 'primordialsea'].includes(blocker) &&
																						!['sunnyday', 'raindance', 'sandstorm', 'hail', 'snowscape', 'chillyreception'].includes(_id15)) {
																					...
																					if (_id15 === 'unboost') {
																						_template64 = this.template(_stat3 ? 'failSingular' : 'fail', 'unboost');
																						return _line25 + _template64.replace('[POKEMON]', this.pokemon(_pokemon31)).replace('[STAT]', _stat3);
																					}

																					_templateId10 = 'fail';
																					if (['brn', 'frz', 'par', 'psn', 'slp', 'substitute', 'shedtail'].includes(_id15)) {
																					...
																					_template64 = this.template(_templateId10, _id15);
																					return _line25 + _template64.replace('[POKEMON]', this.pokemon(_pokemon31));
																				}
																				...
																				case '-mega': case '-primal': {
																					...
																					if (!_id16 && cmd === '-mega' && this.gen < 7) _templateId12 = 'megaGen6';
																					if (!_item2 && cmd === '-mega') _templateId12 = 'megaNoItem';
																					var _template68 = this.template(_templateId12, _id16);
																				...
																				case '-terastallize': {
																					...
																					var _template69 = this.template(_templateId13, _id17);
